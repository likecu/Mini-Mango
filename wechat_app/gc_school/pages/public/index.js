...
Page({
  ...
  numCut: function () {
    if (this.data.express_num > 1) {
      var t = (Number(this.data.form.total) - Number(this.data.form.unit_price)).toFixed(2),
        e = this.data.express_num,
        s = Number(Number(this.data.extra_jian_price) - Number(this.data.form.unit_price)).toFixed(2),
        i = this;
      this.setData({
        form: a({}, i.data.form, {
          total: t
  ...
  formSubmit: function (s) {
    ...
    if (1 == this.data.sysparment.is_attach_show && 1 == this.data.sysparment.is_attach_must && 4 == this.data.form.type && !this.data.attach_file) return e.util.message("请上传附件", "", "error"),
      !1;
    if (console.log("989898980", this.data.t_pic), this.data.t_pic) this.data.t_pic[0];
    else;
  ...
  uploadfile: function () {
    ...
    wx.chooseMessageFile({
      ...
      type: "file",
      success: function (a) {
        console.log(a.tempFiles[0].path);
        var date = new Date();
        var seperator1 = "-";
  ...
  hideModal: function (t) {
    if ("ChooseModal" == this.data.modalName && (console.log("执行这里"), Number(this.data.form.total) < Number(this.data.form.MinRunPrice))) wx.showToast({
      ...
      console.log("优惠金额", this.data.y_money), this.data.y_money > this.data.custom) e.util.message("金额不可低于优惠券金额", "", "error");
    else if (void 0 != this.data.custom)
      if (this.data.custom < 0) e.util.message("输入金额有误", "", "error");
      else {
    ...
    e.util.request({
      ...
      success: function (a) {
        for (var e = [
            [],
            []
          ], s = 0; s < a.data.data.time.length; s++) {
  ...
  uploadimg: function (t) {
    ...
    for (var s = a.data.addedCount, i = 0; i < t.path.length; i++) {
      console.log("走这里", t.path[i]);
      // wx.uploadFile({
      //   url: e.util.url() + "c=entry&a=wxapp&do=ImgPost&m=gc_school",
      //   filePath: t.path[i],
      //   name: "file",
      ...
      wx.cloud.uploadFile({
        ...
        s++;
        var i = a.data.picList;
        i.push(res1.fileID), console.log(i);
        a.setData({
          images: i,
          picList: i,
          addedCount: s
