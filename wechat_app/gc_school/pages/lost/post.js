...
Page({
  ...
  uploadimg: function (t, a, o) {
    ...
    for (var i = this, s = i.data.addedCount, d = 0; d < t.length; d++) {
      // wx.uploadFile({
      ...
      wx.cloud.uploadFile({
        ...
        i.setData({
          ...
          addedCount: s,
          picList: a
        }), console.log(s), s == i.data.picList.length && wx.hideLoading({});
      }).catch((e) => {
        console.log("======上传失败======", result);
        wx.hideLoading();
      });
  ...
  formSubmit: function (t) {
    ...
    var a = t.detail.value.place.replace(/\s+/g, ""),
      o = t.detail.value.phone.replace(/\s+/g, ""),
      i = t.detail.value.remark.replace(/\s+/g, ""),
      s = t.detail.value.date.replace(/\s+/g, ""),
      d = t.detail.value.method.replace(/\s+/g, ""),
      l = /^1[3456789]\d{9}$/;
    if (0 != this.data.cate)
      if ("" != this.data.goods)
        if ("" != a)
    ...
    else e.util.message("请输入正确的手机号", "", "error");
    else e.util.message("请输入地点", "", "error");
    else e.util.message("请输入物品", "", "error");
    else e.util.message("请选择分类", "", "error");
  },
  onShow: function () {},
  onHide: function () {},
  onUnload: function () {},
