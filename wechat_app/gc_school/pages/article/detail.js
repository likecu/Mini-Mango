...
Page({
    ...
    submit: function() {
        ...
        if ("" != this.data.input) {
            var e = this;
            e.data.is_click ? (e.setData({
                is_click: !1
            }), t.util.request({
                url: "entry/wxapp/articleComment",
                data: {
    ...
    onShareAppMessage: function() {
        ...
        t.util.request({
            ...
            success: function(t) {
                return "image" == e.data.detail.media_type ? {
                    title: e.data.detail.content,
                    query: "/gc_school/pages/article/detail?id=" + e.data.id,
                    imageUrl: e.data.detail.img[0]
                } : {
                    title: e.data.detail.content,
                    query: "/gc_school/pages/article/detail?id=" + e.data.id
                };
    ...
    onShareTimeline: function() {
        return console.log(this.data), console.log(this.data.detail), "image" == this.data.detail.media_type ? {
            title: this.data.detail.content,
            query: "id=" + this.data.id,
            imageUrl: this.data.detail.img[0]
        } : {
            title: this.data.detail.content,
            query: "id=" + this.data.id
        };
