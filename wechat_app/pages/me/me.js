...
Page({
  ...
  getUserInfo1: function(t) {
    console.log("打印", t);
    var e = this;
    wx.getUserProfile({
        desc: "用于获取用户信息",
        success: function(result) {
            console.log("用户信息", t.errMsg)
            wx.setStorageSync("token", result.data.data);
            wx.setStorageSync("userInfo", t.detail.userInfo);
        }
    });
},
  updateUser: function(fun_e){
    wx.navigateTo({
      url: '/pages/update_user/update'
    });
  },



  getUserInfo: function (fun_e) {
    let that = this;
    if (fun_e.detail.userInfo == null) {
      wx.showToast({
        title: '取消',
        icon: "none"
      })
    ...
    var listMessage = {
      userNickname: fun_e.detail.userInfo.nickName,
      userGender: fun_e.detail.userInfo.gender,
      userAvatar: fun_e.detail.userInfo.avatarUrl,
      userCity: fun_e.detail.userInfo.city,
      userProvince: fun_e.detail.userInfo.province,
      userCountry: fun_e.detail.userInfo.country,
    }
    wx.login({
      success: function (res) {
        ...
        wx.request({
          ...
          success: function (result) {
            ...
            console.log(result)
            wx.setStorageSync("token", result.data.data);
            wx.setStorageSync("userInfo", fun_e.detail.userInfo);
            that.setData({
              isShow: false,
            })
            wx.hideLoading()
